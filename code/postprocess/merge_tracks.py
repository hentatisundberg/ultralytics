...
from functions import euclidean, create_connection, merge_tracks, associate_points_before, associate_points_within, calc_stats2, modify_output, plot_tracks, prep_data, insert_to_db, predict_from_classifier, df_from_db, plot_tracks2
...
for date in dates: 
    ...
    orig_data = df_from_db("inference/Inference_raw_nomerge.db", f'Ledge == "FAR3"', f'strftime("%Y-%m-%d", time2) == "{date}"')
