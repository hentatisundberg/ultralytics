...
for date in dates: 
    ...
    orig_data = df_from_db("inference/Inference_raw_nomerge.db", f'Ledge == "TRI6"', f'strftime("%Y-%m-%d", time2) == "{date}"', False)
    ...
    if len(orig_data) > 0: 
        ...
        print(stats)
        insert_to_db(v6, "inference/Inference_raw_mergeTRI6.db")
        insert_to_db(stats, "inference/Inference_stats_mergeTRI6.db")
